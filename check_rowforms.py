        ...
        if m:
            ...
            while j < len(lines):
                ...
                for rf in rowform_pattern.findall(lines[j]):
                    if "_" in rf:  # avoid accidental numbers
...
def validate_ttl(ttl_path):
    ...
    for rc_qname, rowforms in rowclasses.items():
        ...
        extra = expected_forms - rowforms_tuples  # These are not errors, only for completeness

        if not missing:
            print("  ✔ All rowForms consistent with interval pattern.")
        else:
            print("  ❌ Incorrect or inconsistent rowForms:")
            for bad in missing:
                print("     →", fmt(list(bad)))

        print(f"  Total rowForms in TTL: {len(rowforms_tuples)}")
        print(f"  Total rowForms derivable: {len(expected_forms)}")
