...
OUTPUT_FILE = "TestSubgraph_ManifestedIn_with_bags.ttl.ttl"
START_URI = "http://music-history-graph.ch/twelve-tone-onto#manifestedIn"
DEPTH = 0  # BFS-Tiefe
...
while queue:
    ...
    # --- 1) Ausgehende Tripel  (current_node als Subjekt) ---
    ...
    # --- 2) Eingehende Tripel  (current_node als Objekt) ---
    ...
    # ⭐ NEW: 3) Tripel, in denen current_node als Prädikat vorkommt
    for s, p, o in g.triples((None, current_node, None)):
        # p ist hier "current_node"
        if current_node == RDF.type:
            continue

        s2 = materialize_bnode(s) if isinstance(s, BNode) else s
        o2 = materialize_bnode(o) if isinstance(o, BNode) else o

        subgraph.add((s2, current_node, o2))
