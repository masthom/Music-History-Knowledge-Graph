...
def parse_ttl_block(block):
    ...
    # Finde die letzte Property-Zeile (vor dem abschließenden Punkt)
    lines = block.split('\n')
    for i, line in enumerate(lines):
        if line.strip() == '.':
            data['last_property_line'] = i - 1
            break
    if data['last_property_line'] == -1:
        data['last_property_line'] = len(lines) - 1
...
def fix_ttl_punctuation(lines):
    """Korrigiert die Zeichensetzung in TTL-Blöcken"""
    fixed_lines = []
    in_subject = True
    ...
    for i, line in enumerate(lines):
        ...
        # Behandle die erste Zeile (Subject)
        if i == 0 and stripped:
            fixed_lines.append(line)
            continue
            
        # Ignoriere leere Zeilen und Kommentare für die Zeichensetzung
        if not stripped or stripped.startswith('#'):
            fixed_lines.append(line)
            continue
            
        # Behandle die letzte Zeile (muss mit . enden)
        if i == len(lines) - 1:
            if stripped != '.':
                # Entferne eventuelle ; am Ende und ersetze durch .
                line = line.rstrip().rstrip(';') + ' .'
            fixed_lines.append(line)
            continue
            
        # Normale Property-Zeilen müssen mit ; enden
        next_line = lines[i + 1].strip() if i + 1 < len(lines) else ''
        if (stripped and not stripped.startswith('#') and 
            not stripped.endswith(';') and not stripped.endswith('.') and
            not stripped.endswith(',') and next_line != '.'):
            line = line.rstrip() + ' ;'
...
def update_ttl_block(block_data, json_entry):
    ...
    for i, line in enumerate(updated_lines):
        ...
        elif 'schema:birthDate' in line and not stripped.startswith('#') and json_entry.get('birthDate'):
        ...
        elif 'schema:birthPlace' in line and not stripped.startswith('#') and json_entry.get('birthPlace'):
        ...
        elif 'schema:deathDate' in line and not stripped.startswith('#') and json_entry.get('deathDate'):
        ...
        elif 'schema:deathPlace' in line and not stripped.startswith('#') and json_entry.get('deathPlace'):
    ...
    updated_lines = fix_ttl_punctuation(updated_lines)
...
def main():
    ...
    with open('music_graph_updated.ttl', 'w', encoding='utf-8') as f:
        f.write('\n\n'.join(updated_blocks))
