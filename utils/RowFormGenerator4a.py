...
def generate_ttl_output_from_forms(forms):
    ...
    output.append("\n# RowForm-Deklarationen")
    for row in prime_forms:
        name = format_row_name(row)
        output.append(f"mhg:rf_{name} a mhg:rowForm ;")
        output.append(f"    mhg:hasIntervalPattern mhg:ip_{p_pattern} ;")
        output.append(f"    mhg:hasRowClass mhg:rc_{p_pattern} .")

    for row in inversion_forms:
        name = format_row_name(row)
        output.append(f"mhg:rf_{name} a mhg:rowForm ;")
        output.append(f"    mhg:hasIntervalPattern mhg:ip_{i_pattern} ;")
        output.append(f"    mhg:hasRowClass mhg:rc_{p_pattern} .")

    for row in retrograde_forms + retrograde_inversion_forms:
        name = format_row_name(row)
        output.append(f"mhg:rf_{name} a mhg:rowForm ;")
        output.append(f"    mhg:hasIntervalPattern mhg:ip_{r_pattern} ;")
        output.append(f"    mhg:hasRowClass mhg:rc_{p_pattern} .")

    for row in retrograde_inversion_forms:
        name = format_row_name(row)
        output.append(f"mhg:rf_{name} a mhg:rowForm ;")
        output.append(f"    mhg:hasIntervalPattern mhg:ip_{ri_pattern} ;")
        output.append(f"    mhg:hasRowClass mhg:rc_{p_pattern} .")
