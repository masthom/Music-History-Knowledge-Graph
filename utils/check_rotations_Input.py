...
def process_single_pattern(input_pattern, output_ttl_path=None):
    ...
    if input_pattern.startswith('ip_'):
        pattern = input_pattern[3:]  # Entferne "ip_"
    else:
...
def interactive_mode():
    ...
    while True:
        ...
        try:
            ...
            if user_input.startswith('ip_'):
                pattern_parts = user_input[3:].split('_')
            else:
                pattern_parts = user_input.split('_')
            
            # Prüfe, ob alle Teile Zahlen sind
            for part in pattern_parts:
                int(part)
            
            # Verarbeite das Pattern
            result = process_single_pattern(user_input)
            
            print("\n" + "="*50 + "\n")
            
        except ValueError as e:
            print(f"Ungültiges Format: {e}")
            print("Bitte geben Sie ein gültiges Pattern ein (z.B. '1_2_3')")
