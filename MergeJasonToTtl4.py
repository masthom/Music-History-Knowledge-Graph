...
def fix_ttl_punctuation(lines):
    """Korrigiert die Zeichensetzung in TTL-Blöcken - vereinfachte Version"""
    if not lines:
        return lines
    ...
    # Gehe alle Zeilen durch und korrigiere die Zeichensetzung
    for i, line in enumerate(lines):
        stripped = line.strip()
        ...
        # Letzte Inhaltszeile muss mit Punkt enden
        if i == last_content_line:
            # Entferne eventuelle vorhandene Semikolons oder Punkte am Ende
            clean_line = line.rstrip().rstrip(';').rstrip('.')
            fixed_lines.append(clean_line + ' .')
        else:
            # Andere Inhaltszeilen müssen mit Semikolon enden (wenn nicht schon vorhanden)
...
def update_ttl_block(block_data, json_entry):
    ...
    for i, line in enumerate(block_data['raw_lines']):
        original_line = line
        stripped = line.strip()
        ...
        # Überprüfe und aktualisiere birthDate
            ...
            if current_date and current_date != json_date.replace('"^^xsd:date', ''):
                ...
                updated_lines.append(f"    # ORIGINAL: {original_line.strip()}")
    ...
    # Sicherstellen, dass es nur einen Punkt am Ende gibt
    if updated_lines and updated_lines[-1].strip() == '.':
        # Entferne alle weiteren Punkte am Ende
        while len(updated_lines) > 1 and updated_lines[-2].strip() == '.':
            updated_lines.pop(-2)
...
def main():
    ...
    # Lade TTL-Datei
    with open('composers_updated.ttl', 'r', encoding='utf-8') as f:
        ttl_content = f.read()
